...
\author{Thomas~Pumir,~%~\IEEEmembership{Student Member,~IEEE,}
        ...
        and~Alexandre~M.~Bayen%,~\IEEEmembership{Member,~IEEE}% <-this % stops a space
\thanks{T.~Pumir and A.~Bayen are with the Department
of Electrical Engineering and Computer Science, University of California, Berkeley,
CA; L.~Anderson is with the Department of Civil and Environmental Engineering, University of California, Berkeley. Corresponding e-mail: thomas.pumir@berkeley.edu}% <-this % stops a space
\thanks{This work was sponsored by... }% <-this % stops a space
}
...
\section{Conclusion}
In this work we have defined three extensions of the max pressure controller required in order to be applied to a real network of signalized traffic intersections. Given only the constraint that the network demands are (in average) serviceable, we have proven that updating a max pressure controller at a slower rate than that which governs traffic flow will not destroy the stabilizing properties of the controller. We have also formulated a max pressure controller which allocates a fixed minimum proportion of service to each permissible phase, and shown that such a controller will also stabilize a network.

While there are additional modifications to the control model that must be addressed before max pressure can be considered a feasible algorithm for real-time operation of traffic signal controllers, our implementation in the rfeerence micro-simulation platform demonstrates how it could comply with the hardware and communications constraints commonly encountered in existing roadway infrastructure. These numerical simulations furthermore reveal that an implementation of cycle-allocated max pressure competes with the existing standard feedback controller in many performance metrics, especially during periods of high congestion. This comes on top of theoretical proofs it provides, unlike algoriithms used in practice. It also appears to provide less variance in delays than the existing alternative. 
...
%\appendices
%\section{Proof of stability: immediate feedback controller}
%\input{appendix_immediatefeedbackproof}

% you can choose not to have a title for an appendix
% if you want by leaving the argument blank
%\section{Proof of stability: cycle controller}
%\input{appendix_cycleproof}

%\section{Proof of stability: allocated controller}
%\input{appendix_distributedproof}

% use section* for acknowledgement
%\section*{Acknowledgment}

The authors would like to thank Pravin Varaiya for his insights on this work, as well as Brian Peterson and Joe Butler for their support for the implementation.
