...
\end{itemize}
Note that there is necessarily a linear relationship between the expected link flow vector $f$ and the boundary demand vector $d$: $f=dP$ where the matrix $P$ depends only on expected routing proportions within the network. 
...
\end{equation}
Also, define $conv(U)$ as the convex hull of the set of permissible control matrices $U$. 

The following properties are shown in \cite{MaxPressureStochastic}: 
\begin{Prop}
...
%\end{equation}
\begin{Prop}
A demand is \emph{feasible} if and only if  $\exists \; M_{\overline S} \in conv(U)$ and $ \varepsilon > 0$ such that
 \begin{equation}  \label{feasible_demand} 
 c(l,m)M_{\overline S}(l,m) > f_{l}r(l,m) + \varepsilon. 
 \end{equation}
\end{Prop}
Define $D^0$ to be the set of all average demand vectors $\{d_l\}$ that satisfy \eqref{feasible_demand} and are therefore feasible.
